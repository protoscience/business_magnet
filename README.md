...
- **OpenSSF Scorecard** (security-posture checks, weekly — findings in Security tab)

Dependencies are kept current via **Dependabot** (weekly PRs for pip packages and GitHub Actions). Scan results are visible under the repo's *Security* tab; the badge above shows the latest status.
